 ...
 Script.on('load', function () {
    ...
    Database.readLastPrimaryKey("Rodent","Devices",function(eventData){
     ...
     downloadDevData.subscribe("pressed", ()=>{
		 ...
		 console.log("Table data is " + JSON.stringify(formData.devices.data, null, 4));
		 if(curentSelection != 1) { // 1
			 return;
		 }
		 //var ids = formData.products.getColumn("id");
		 var ids = formData.devices.getColumn("id");
        ...
        var data = {};
         ...
         var devSensaCollection = new SensaCollection(column, column[0], {data:JSON.parse(JSON.stringify(data))});
     ...
     reader.onload = function (event) {
         ...
         for (var i = 1; i < dataLines.length; i++) {
             ...
             if (row.length > 1 && row[0] != "") {
                 ...
                 dbRec.Region = row[1];
                 dbRec.Model = row[2];
                 dbRec.BatchNumber = row[3];
                 
                 
                 dbRec.HWVersion = row[4];
                 dbRec.Location = row[5];
                 dbRec.Status = 0;
